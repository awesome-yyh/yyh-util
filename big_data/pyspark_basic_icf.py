...
def getSimilarItems(begin_date, end_date, bicf_filename):
    ...
    inter_read_df = inter_read_df.withColumn("click", click) \
                                 ...
                                 .selectExpr("keyword", "id_type", "click * 1 as feature")
    inter_read_df.persist(StorageLevel.MEMORY_AND_DISK_SER)  # 缓存
    ...
    single_df.persist(StorageLevel.MEMORY_AND_DISK_SER)  # 缓存
    ...
    pair_df = pair_df.join(single_df1, "doc1", "left") \
                     ...
                     .withColumn("similar", fun.col("feature"), fun.sqrt(fun.col("feature1") * fun.col("feature2")))
...
def timedJob():
    ...
    begin_date = (datetime.datetime.now() + datetime.timedelta(days=-7)).strftime("%Y%m%d")
