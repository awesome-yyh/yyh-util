...
# 主函数
def getAlsEmbedding(begin_date, end_date, query_filename, doc_filename):
  ...
  # 合并
  index_df = index_df.groupBy(["keyword","queryIndex","id_type","item_type","docIndex"]) \
            .agg(fun.sum("duration").alias("dur_sum"), \
  ...
  query_embedding_df = query_index_embedding_df.join(index_df, query_index_embedding_df.id == index_df.queryIndex) \
                                              .select("id","keyword","features")
  ...
  doc_embedding_df = doc_index_embedding_df.join(index_df, doc_index_embedding_df.id == index_df.docIndex) \
                                            .select("id","id_type","features")
...
if __name__ == "__main__":
  ...
  startTime = (datetime.datetime.now() + # -1是昨天，-7是一周
