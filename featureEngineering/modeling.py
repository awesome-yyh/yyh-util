...
def UserDefineBertFuntion(config,
           is_training,
           input_ids,
           input_mask=None,
           token_type_ids=None,
...
class BertModel4Query(BertModel):
  """BERT model ("Bidirectional Embedding Representations from a Transformer").
  ...
  def __init__(self,
    ...
    with tf.compat.v1.variable_scope("bert4query", scope):
      with tf.compat.v1.variable_scope("embeddings4query"):
        # Perform embedding lookup on the word ids.
        (self.embedding_output, self.embedding_table) = embedding_lookup(
            input_ids=input_ids,
            vocab_size=config.vocab_size,
            embedding_size=config.hidden_size,
            initializer_range=config.initializer_range,
            word_embedding_name="word_embeddings4query",
            use_one_hot_embeddings=use_one_hot_embeddings)
        ...
        self.embedding_output = embedding_postprocessor(
            ...
            token_type_ids=token_type_ids,
            token_type_vocab_size=config.type_vocab_size,
            token_type_embedding_name="token_type_embeddings4query",
            use_position_embeddings=True,
            position_embedding_name="position_embeddings4query",
            initializer_range=config.initializer_range,
            max_position_embeddings=config.max_position_embeddings,
            dropout_prob=config.hidden_dropout_prob)

      with tf.compat.v1.variable_scope("encoder4query"):
        # This converts a 2D mask of shape [batch_size, seq_length] to a 3D
        # mask of shape [batch_size, seq_length, seq_length] which is used
        # for the attention scores.
        attention_mask = create_attention_mask_from_input_mask(
...
class BertModel4Doc(BertModel):
  ...
  def __init__(self,
    ...
    with tf.compat.v1.variable_scope("bert4doc", scope):
      with tf.compat.v1.variable_scope("embeddings4doc"):
        # Perform embedding lookup on the word ids.
        (self.embedding_output, self.embedding_table) = embedding_lookup(
            input_ids=input_ids,
            vocab_size=config.vocab_size,
            embedding_size=config.hidden_size,
            initializer_range=config.initializer_range,
            word_embedding_name="word_embeddings4doc",
            use_one_hot_embeddings=use_one_hot_embeddings)
        ...
        self.embedding_output = embedding_postprocessor(
            ...
            token_type_ids=token_type_ids,
            token_type_vocab_size=config.type_vocab_size,
            token_type_embedding_name="token_type_embeddings4doc",
            use_position_embeddings=True,
            position_embedding_name="position_embeddings4doc",
            initializer_range=config.initializer_range,
            max_position_embeddings=config.max_position_embeddings,
            dropout_prob=config.hidden_dropout_prob)

      with tf.compat.v1.variable_scope("encoder4doc"):
        # This converts a 2D mask of shape [batch_size, seq_length] to a 3D
        # mask of shape [batch_size, seq_length, seq_length] which is used
        # for the attention scores.
        attention_mask = create_attention_mask_from_input_mask(
...
def get_assignment_map_from_checkpoint(tvars, init_checkpoint):
  ...
  for x in init_vars:
    ...
    if name not in name_to_variable:
      continue
    # 重新载入原生的bert的模型（未经过fine-tune，增加或者未增加其他的NN网络） 需要使用下边这句
    # assignment_map[name] = name
    # 重新载入自己修改后的模型(经过fine-tune，增加了其他的NN网络) 需要使用下边这句，下边这句兼容上边这句
