...
# 使用2个Dense层对mnist手写数字识别(subclassing模式)
...
class MnistModel(tf.keras.Model):
    ...
    def __init__(self, dim, num_classes=10):
        ...
        self.flatten = tf.keras.layers.Flatten(input_shape=(dim))
        self.dense1 = tf.keras.layers.Dense(128, activation=tf.nn.relu,
                                            kernel_regularizer=tf.keras.regularizers.L2(0.002), # 对该层的权重正则
                                            activity_regularizer=tf.keras.regularizers.L2(0)) # 对该层的输出矩阵正则
...
dim = (28,28)
model = MnistModel((dim), num_classes=10)
# 查看模型结构
# model.build((None,*dim))
model.build_graph().summary()
tf.keras.utils.plot_model(model.build_graph(), "model.png", show_shapes=True)
