...
# 使用2个Dense层对mnist手写数字识别(function模式)
...
# 模型加载和预测(pb)
restored_saved_model=tf.keras.models.load_model(pb_file_path) # 加载keras模型
# restored_saved_model=tf.saved_model.load(pb_file_path) # 加载其他模型
